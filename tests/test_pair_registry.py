...
async def test_get_allowed_fetches_and_caches() -> None:
    ...
    reg = PairRegistry(src, limit=30, ttl_sec=100)
    r1 = await reg.get_allowed(now=0.0)
    assert "SOL/USDT:USDT" in r1
    # TTL 내 재호출은 캐시 사용 — 거래소 조회 안 함.
    await reg.get_allowed(now=50.0)
    assert src.calls == 1
    # TTL 만료 → 재조회.
    await reg.get_allowed(now=200.0)
    assert src.calls == 2
...
async def test_majors_always_included() -> None:
    ...
    reg = PairRegistry(src)
    r = await reg.get_allowed(now=0.0)
...
async def test_fetch_failure_keeps_existing_cache() -> None:
    ...
    reg = PairRegistry(src, ttl_sec=10)
    r1 = await reg.get_allowed(now=0.0)
    assert "SOL/USDT:USDT" in r1
    # 조회 실패 + TTL 만료 → 기존 캐시 유지.
    src.fail = True
    r2 = await reg.get_allowed(now=100.0)
...
async def test_is_allowed() -> None:
    src = _FakeSource(["SOL/USDT:USDT"])
    reg = PairRegistry(src)
    assert await reg.is_allowed("SOL/USDT:USDT", now=0.0)
    assert await reg.is_allowed("BTC/USDT:USDT", now=0.0)
    assert not await reg.is_allowed("NOPE/USDT:USDT", now=0.0)


@pytest.mark.asyncio
async def test_limit_passed_to_source() -> None:
    src = _FakeSource([f"C{i}/USDT:USDT" for i in range(50)])
    reg = PairRegistry(src, limit=10)
    r = await reg.get_allowed(now=0.0)
