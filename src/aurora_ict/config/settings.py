...
# 2026-07-20 Origo 2.1 = 2.0 + 품질 기반 자금배분 (#SMART-SIZE, FST#7). 진입 시
...
class IctSettings(BaseSettings):
    ...
    # 2026-08-06 #MMBM-WIRE: 2번째 진입모델(마켓메이커 반전) 활성 플래그.
    # ⚠️ 7/21 에 "Origo 2.2 = 2.1 + MMBM 활성화" 로 배포했으나 **이 플래그를 봇에
    #    넘기는 코드가 없어 2주간 한 번도 돌지 않았다**(bot.mmbm_enabled 기본 False).
    #    구현·테스트·모델명은 다 있었는데 배선 한 줄이 빠진 케이스.
    # 재검증(2026-08-06, 라이브 정합 하니스 · BTC+ETH · 7x · 복리):
    #     SB 단독    45건 · 월 0.77 · 건당 0.686R · 부트중앙  5.78배 · 낙폭 36.7%
    #     MMBM 단독 862건 · 월 14.5 · 건당 0.199R · 부트중앙  8.94배 · 낙폭 67.1%
    #     결합      907건 · 월 15.3 · 건당 0.223R · 부트중앙 37.83배 · 낙폭 69.5%
    #   · SB 와 **중복 0건** — 같은 셋업을 헐겁게 잡는 게 아니라 다른 기회를 잡는다.
    #   · 건당 R 95% 구간 [+0.130 ~ +0.323] 으로 0 초과 확정, 연도 6/6·롱숏 모두 흑자.
    #   · 파산확률 0% 유지, 최악 5% 시나리오는 오히려 2.27→6.04배로 개선.
    # 빈도(월 0.77건)가 상용화 최대 약점이었고 confluence 문턱 완화로는 해결되지
    # 않았다(5→4 추가분 승률 25%). 모델 추가가 정답이었다.
    # ⚠️ 구현 한계: 정통 4단계 중 **유동성 스윕(2단계)과 SMT 가 빠져 있다**. 다만
    #    스윕을 요구하면 거래가 29% 줄어 복리 성적이 절반이 된다(부트중앙 9.49→4.65배).
    #    정통성보다 실측을 택한 것 — 파트너 결정 "A→C 순".
    origo_mmbm_enabled: bool = Field(default=False)
    ...
    def _enforce_license_tier_policy(self) -> IctSettings:
        ...
        if self.license_type.startswith("sub_"):
            ...
            if self.leverage > 7:
                self.leverage = 7
            # #MMBM-WIRE: 2번째 진입모델 강제 활성. 빈도가 상용화 최대 약점이었고
