...
## 0. 먼저 알아둘 것 — 등록을 확인한 손절은 서버와 독립적이다

**봇이 죽어도 거래소에 등록된 SL/TP 는 그대로 작동한다.** 다만 등록을 시도했다는
코드나 로그만으로 보호를 확정하면 안 된다. 포지션 SL 또는 조건부 주문을 실제 조회해
확인해야 한다. 등록 실패 뒤 비상청산도 실제 잔량이 0인지 별도로 확인한다.
장애 시 **보호 등록 여부와 조회 실패 여부를 먼저 구분한 뒤**, 멈춘 기능을 판단한다.
2026-09-13 감사에서 미추적 무손절 포지션과 주문/체결 혼동 경로를 확인했다.
`SYSTEM_SAFETY_AUDIT_2026-09-13.md` 참고. Cursus 수정은 사용자 요청으로 제외한다.
...
이 표는 **보호 주문 등록을 확인한 포지션에만 적용**된다. 조회가 실패하면 미확인이지
안전하다는 뜻이 아니다. 손절이 있어도 급변/슬리피지로 손실 상한이 보장되지는 않는다.
...
§0의 표는 거래소 보호 주문을 확인한 범위에만 안내한다. 조회 실패나 보호 누락이
있으면 확인된 계정과 미확인 계정을 구분하고, 미확인을 정상으로 안내하지 않는다.
