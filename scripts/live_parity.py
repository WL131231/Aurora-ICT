...
# [08-13] implied_fvg 종결 — **성적 1위는 손익비 필터의 착시였다**
# 소스 분리에서 implied_fvg 가 건당 +0.508R(본) / +0.319R(홀드, 심볼 4/4)로 1위였다.
# 빈도가 3% 뿐이라 늘릴 수 있는지 보려고 깔때기를 셌더니:
#     검출 34,338 → 시간필터 6,038 → **min_rr 2.0 에서 5,960 탈락** → 진입 68건(0.20%)
#     손익비 분포 중앙 **0.02** · 2.0 이상 **1.1%**
# 몸통 갭이라 구간이 좁은데 ATR 손절 바닥(1.5×)이 손절을 강제로 벌린다 →
# 손익비가 원리상 안 나오고, 통과한 1.1% 는 "셋업이 좋아서"가 아니라
# **"손절이 우연히 가까워서"** 다. 문턱을 낮춰 표본을 늘리자 무너졌다:
#     min_rr 2.0 → 1.5 → 1.2 → 1.0
#     implied  +0.109 → -0.143 → -0.330 → **-0.394 [-0.671 ~ -0.092] 적자확정**
#     (대조군: fvg +1.073→+0.109 양수 유지 · turtle +0.017→+0.124 오히려 상승
#      → 문턱을 낮추면 전부 나빠지는 게 아니라 **이 소스만** 무너진다)
# 구조는 [[research_fib_ote_closed_2026-08-07]] 과 같다 — 좁은 구간 소스는 ATR 손절
# 바닥에 먹혀 R 이 안 나온다. **축 종결.**
# 부수 기록: 같은 실험에서 turtle 이 최근 14개월 54건 기준 +0.017R 로 나왔다(5년
# 3,478건에서는 -0.248R). 작은 표본의 부호 흔들림 — 3단 검증을 거친 제거 판단은 유지.
# 스크립트: implied_funnel_2026-08-11.py · implied_rr_2026-08-12.py
