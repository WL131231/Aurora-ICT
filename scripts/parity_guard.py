...
## 사용법
    ...
    python scripts/parity_guard.py             # 검사 (CI/배포 훅용)
    python scripts/parity_guard.py --update    # 현재 상태를 lock 에 기록
...
# 프로덕션은 **읽기 전용**. 절대 수정하지 않는다.
PROD_ROOT = r"C:\Users\지영민\Desktop\Aurora-ICT"

# 점수 변이가 일어날 수 있는 파일 (프로덕션 상대경로).
TARGETS = [
    r"src\aurora_ict\bot\bot_ict_instance.py",
    r"src\aurora_ict\strategy\silver_bullet.py",
    r"src\aurora_ict\strategy\ict_signal.py",
    r"src\aurora_ict\strategy\mmbm.py",
]

LOCK = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                    "..", "data", "parity", "score_paths.lock.json")
